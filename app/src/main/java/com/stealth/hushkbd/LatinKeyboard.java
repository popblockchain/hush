...
public class LatinKeyboard extends Keyboard {
	...
	public boolean mEnterIsCr = true ; // enter key = CR not special key
	
	public Key mEncryptKey ;
    public Key mEnterKey;
    private Key mSpaceKey;
    public Key mGroupIdKey ;
    public Key mSettingsKey ;
    /**
    ...
    protected Key createKeyFromXml(Resources res, Row parent, int x, int y, 
        ...
        else if (key.codes[0] == LatinKeyboardView.KEYCODE_LANGUAGE_ENCRYPT)
        	mEncryptKey = key ;
        else if (key.codes[0] == LatinKeyboardView.KEYCODE_GROUP_ID)
        	mGroupIdKey = key ;
        else if (key.codes[0] == LatinKeyboardView.KEYCODE_SETTINGS)
        	mSettingsKey = key ;
        else if (key.codes[0] == ' ') {
            mSpaceKey = key;
